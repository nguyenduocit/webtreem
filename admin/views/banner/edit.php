<?php
  ...
  */
  ...
  $db = new viewAdd();
  // lấy ra các danh mục có parenr_id = 0
  $parent = $db->fetchwhere('category','parent_id = 0');

  // kiểm tra có tồn tại biến GET và gán dữ liệu
  if(testInputInt($_GET['id']))
  {
    $id = testInputInt($_GET['id']); 

    $where = "id = ".$id;
    // lấy ra danh mục có id = $id
    $data = $db->fetchwhere ('category',$where);
    // nếu k có giá trị trả về thì thông báo lỗi 
    if (empty($data)) {
      # code...
  ...
  else
  {
    ...
    // không tồn tại biến id thì chuyển người dùng về trang chủ
                  ...
                  <table class="table">
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                          <input type="text" name="icon" class="form-control" id="inputEmail3" placeholder="icon" value="<?php echo $value['icon'] ?>" >
